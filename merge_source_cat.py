...
def load_detector(data_ref,
    ...
    #print("psf_flux: ", cat[flux_names['psf_flux']].values, " psf_flux_err: ", cat[flux_names['psf_flux_err']].values)

    psf_flux_val = cat[flux_names['psf_flux']].values
    psf_flux_err_val = cat[flux_names['psf_flux_err']].values

    mag = np.ndarray(shape=(len(psf_flux_val)))
    mag_err = np.ndarray(shape=(len(psf_flux_val)))

    for i in range(0, len(psf_flux_val)):
        meas = calib.instFluxToMagnitude(psf_flux_val[i],
                                                       psf_flux_err_val[i])
        mag[i] = meas.value
        mag_err[i] = meas.error
